...
description: "What it is and how to do it in RStudio and PLINK"
...
On the [home page](https://statgensummary.netlify.app), I introduced **genetic variants**. We can study these genetic variants with something known as a **Genetic - Wide Association Study (GWAS)**. The overarching goal of a GWAS is to help us understand which SNPs might be causally associated with our trait of interest, which can be particularly important and helpful when trying to find a cure for a disease or understand why certain people get that disease.

![](images/gwas.png)

Using the trait we developed in the [Data](https://statgensummary.netlify.app/data.html) tab, we can implement **marginal regression** to run GWAS on our data. If you haven't yet looked through the Data section, it would be useful to do now in order to understand the context of this GWAS. 
...
Each of these models produces an estimate for the coefficient on the SNP. For example, the coefficient for `snp1mod` is 1.08. The way we might interpret this is that for every additional minor allele (G for example) that you carry at that position, the trait of interest changes by about 1.08 units. If the trait we were measuring was height, we would expect your height to increase about 1.08 inches for every additional minor allele (a value of either 0, 1, or 2) at SNP 1. This is obviously pretty extreme, but you understand the idea.
...
I then selected the `pedigree` and `member` columns from the `rbloggers_fam` table and column-binded on a trait that varies by population. I write this new dataset to the folder where the data is stored.
...
In PLINK, I then run the command `./plink --bfile rbloggersComb --assoc --adjust --pheno rbloggers_poptrait --out as2`. The command and subsequent output are shown below.
...
Overall, this manhattan plot looks fairly similar to the original one we created earlier. However, the most significant p-value in this plot has a $-\text{log}_{10}(\text{p-value})$ closer to 6 instead of 8. This is expected given we did not simulate the trait based on any one particular SNP and we know that none of the SNPs are causally located with our trait of interest. As previously mentioned, in a real genetic study we do not know whether or not any of our SNPs will be causally associated with our trait of interest. Therefore, we need some kind of threshold to determine if what SNPs we should look more closely at (if any). To learn more about how to do this, click on the [Multiple Testing](https://statgensummary.netlify.app/multipletesting.html) tab! 
